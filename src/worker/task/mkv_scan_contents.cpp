...
void mkv_scan_contents(sonarr_type &type, context &context)
{
    ...
    // copy out the information we care about..tracks..languages...codecs

    try
    {
        StdIOCallback Cluster_file(fullPath.generic_string().c_str(), MODE_READ);
            ...
            if (EbmlId(*ElementLevel0) == EBML_ID(KaxSegment))
            {
                ...
                while (ElementLevel1 != NULL)
                {
                    ...
                    else if (EbmlId(*ElementLevel1) == EBML_ID(KaxTracks))
                    {
                        ...
                        ElementLevel2 =
                          aStream.FindNextElement(EBML_CONTEXT(ElementLevel1), UpperElementLevel, 0, bAllowDummy);
                        ...
                        while (ElementLevel2 != NULL)
                        {
                            ...
                            /// \todo switch the type of the element to check if it's one we want to handle, like
                            /// attachements
                            if (EbmlId(*ElementLevel2) == EBML_ID(KaxTrackEntry))
                            {
                                printf("* Found a track\n");

                                ElementLevel3 = aStream.FindNextElement(
                                  EBML_CONTEXT(ElementLevel2), UpperElementLevel, 0, bAllowDummy);
                            ...
                            else
                            {
                                ...
                                ElementLevel2 = aStream.FindNextElement(
                                  EBML_CONTEXT(ElementLevel1), UpperElementLevel, 0, bAllowDummy);
                            }
                        }
                    }
                            ...
                            else if (EbmlId(*ElementLevel2) == EBML_ID(KaxBlockGroup))
                            {
                                ...
                                if (DataBlock != NULL)
                                {
                                    ...
                                    printf("   Track # %d / %d frame%s / Timecode %I64d\n",
                                      DataBlock->TrackNum(),
                                      DataBlock->NumberFrames(),
                                      (DataBlock->NumberFrames() > 1) ? "s" : "",
                                      DataBlock->GlobalTimecode());
                                }
                                else
                                {
                                    printf("   A BlockGroup without a Block !!!");
                                }
                                KaxBlockDuration *BlockDuration =
                                  static_cast<KaxBlockDuration *>(aBlockGroup.FindElt(EBML_INFO(KaxBlockDuration)));
                                if (BlockDuration != NULL)
                                {
                                    printf("  Block Duration %d scaled ticks : %ld ns\n",
                                      std::uint32_t(*BlockDuration),
                                      std::uint32_t(*BlockDuration) * TimecodeScale);
                                }
                                KaxReferenceBlock *RefTime =
                                  static_cast<KaxReferenceBlock *>(aBlockGroup.FindElt(EBML_INFO(KaxReferenceBlock)));
                                if (RefTime != NULL)
                                {
                                    printf("  Reference frame at scaled (%d) timecode %ld\n",
                                      std::int32_t(*RefTime),
                                      std::int32_t(std::int64_t(*RefTime) * TimecodeScale));
                                }
     ...
     /// \todo There should be a way to get the default values of the elements not defined
                                ...
                                while (ElementLevel3 != NULL)
                                {
                                    ...
                                    if (EbmlId(*ElementLevel3) == EBML_ID(KaxBlock))
                                    {
                                        ...
                                        {
                                            printf("   [%s]\n",
                                              DataBlock.GetBuffer(i)
                                                .Buffer());// STRING ONLY POSSIBLE WITH THIS PARTICULAR EXAMPLE (the
                                                           // binary data is a string)
                                        }
#endif// NO_DISPLAY_DATA
      //                  printf("Codec ID   : %s\n", &binary(CodecID)); // strings
      //                  for the moment (example)
                                    }
                                    ...
                                    {
                                        KaxBlockDuration &BlockDuration =
                                          *static_cast<KaxBlockDuration *>(ElementLevel3);
                    ...
                    else if (EbmlId(*ElementLevel1) == EBML_ID(KaxCues))
                    {
                        ...
                        CuesEntry->Read(aStream,
                          EBML_CLASS_CONTEXT(KaxCues),
                          UpperElementLevel,
                          ElementLevel2,
                          bAllowDummy);// build the entries in memory
                        ...
                        for (Index0 = 0; Index0 < CuesEntry->ListSize(); Index0++)
                        {
                            if (EbmlId(*(*CuesEntry)[Index0]) == EBML_ID(KaxCuePoint))
                            {
                                ...
                                for (Index1 = 0; Index1 < CuePoint.ListSize(); Index1++)
                                {
                                    ...
                                    {
                                        KaxCueTrackPositions &CuePos =
                                          *static_cast<KaxCueTrackPositions *>(CuePoint[Index1]);
                                        printf("  Positions\n");
                                        ...
                                        for (Index2 = 0; Index2 < CuePos.ListSize(); Index2++)
                                        {
                                            ...
                                            {
                                                KaxCueClusterPosition &CuePoss =
                                                  *static_cast<KaxCueClusterPosition *>(CuePos[Index2]);
                                                printf("   Cluster position %d\n", std::uint64_t(CuePoss));
                                            }
                                            else if (EbmlId(*(CuePos[Index2])) == EBML_ID(KaxCueReference))
                                            {
                                                KaxCueReference &CueRefs =
                                                  *static_cast<KaxCueReference *>(CuePos[Index2]);
                                                printf("   Reference\n");
                                                ...
                                                for (Index3 = 0; Index3 < CueRefs.ListSize(); Index3++)
                                                {
                                                    ...
                                                    {
                                                        KaxCueRefTime &CueTime =
                                                          *static_cast<KaxCueRefTime *>(CueRefs[Index3]);
                                                        printf("    Time %d\n", std::uint32_t(CueTime));
                                                    }
                                                    else if (EbmlId(*(CueRefs[Index3])) == EBML_ID(KaxCueRefCluster))
                                                    {
                                                        KaxCueRefCluster &CueClust =
                                                          *static_cast<KaxCueRefCluster *>(CueRefs[Index3]);
                    ...
                    else if (EbmlId(*ElementLevel1) == EBML_ID(KaxSeekHead))
                    {
                        ...
                        // read it in memory
                        MetaSeek->Read(
                          aStream, EBML_CLASS_CONTEXT(KaxSeekHead), UpperElementLevel, ElementLevel2, bAllowDummy);
                    ...
                    else if (EbmlId(*ElementLevel1) == EBML_ID(KaxChapters))
                    {
                        ...
                        // read it in memory
                        Chapters->Read(
                          aStream, EBML_CLASS_CONTEXT(KaxChapters), UpperElementLevel, ElementLevel2, bAllowDummy);
                                ...
                                for (Index2 = 0; Index2 < Edition.ListSize(); Index2++)
                                {
                                    if (EbmlId(*(Edition[Index2])) == EBML_ID(KaxChapterAtom))
                                    {
                                        ...
                                        for (Index3 = 0; Index3 < aChapterAtom.ListSize(); Index3++)
                                        {
                                            ...
                                            {
                                                printf("      Display info\n");
                                                KaxChapterDisplay &aDisplay =
                                                  *static_cast<KaxChapterDisplay *>(aChapterAtom[Index3]);
                                                unsigned int Index4;
                                                for (Index4 = 0; Index4 < aDisplay.ListSize(); Index4++)
                                                {
...
#if !defined(__CYGWIN__) && !defined(__APPLE__) && !defined(__BEOS__) && !defined(__NetBSD__)
                                                        wprintf(L"       Display \"%s\"\n",
                                                          UTFstring(*static_cast<EbmlUnicodeString *>(aDisplay[Index4]))
                                                            .c_str());
#else
                                                        printf("       Display \"%s\"\n",
                                                          UTFstring(*static_cast<EbmlUnicodeString *>(aDisplay[Index4]))
                                                    ...
                                                    else if (EbmlId(*(aDisplay[Index4])) == EBML_ID(KaxChapterLanguage))
                                                    {
                                                        printf("       For language \"%s\"\n",
                                                          std::string(*static_cast<EbmlString *>(aDisplay[Index4]))
                                                            .c_str());
                                                    }
                    ...
                    else if (EbmlId(*ElementLevel1) == EBML_ID(KaxTags))
                    {
                        ...
                        // read it in memory
                        AllTags->Read(
                          aStream, EBML_CLASS_CONTEXT(KaxTags), UpperElementLevel, ElementLevel2, bAllowDummy);
                    ...
                    else
                    {
                        ...
                        ElementLevel1 =
                          aStream.FindNextElement(EBML_CONTEXT(ElementLevel0), UpperElementLevel, 0, bAllowDummy);
                    }
                }
            }
        }
